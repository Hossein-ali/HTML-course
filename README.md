...
| Syntax              | Description |
| -----------         | ----------- |
| `attribute class = ""`   | داشته باشد`class` هر تگ و المان میتواند چند   |
| `attribute id = ""`|  داشته باشد`id`هر تگ یا المان میتواند فقط یک |

**
توجه : تفاوت `clss` با `id` در استایل دهی css  مشخص میشود.
...
| `<q></q>`|  اما در میان یک خط`blockquote`همانند تگ|
